...
const spaceGenerator = (direction, width) => {
	...
	for (let i = 1; i < 14; i++) {
		if (direction === 'horizontal') {
			row.push(<div style={{ width: size, height: 43, border: '3px solid black', borderLeft: 0 }}>{i}</div>)
		} else {
			row.push(<div style={{ width: size, height: 43, border: '3px solid black', borderTop: 0 }}>{i-1}</div>)
		}
	}

	if (direction === 'vertical') {
		row.push(<div style={{ width: size, height: 43, border: '3px solid black', borderTop: 0, borderBottom: 0 }}>13</div>)
	}
